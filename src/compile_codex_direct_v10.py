...
def dimension_item(
    ...
    score: int,
    *,
    source_salience_policy: str,
) -> dict[str, Any]:
    if (
        dimension == "source_salience"
        and source_salience_policy == "fixed_neutral_2_no_longform_overview"
    ):
        reason = (
    ...
    *,
    ...
    editorial = {
        dimension: dimension_item(
            ...
            source_salience_policy=source_salience_policy,
        )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compile frozen Codex direct v10 pointwise assessments."
    )
    ...
    parser.add_argument(
        "--source-salience-policy",
        choices=(
            "fixed_neutral_2_no_longform_overview",
            "evaluated_from_compact_longform_overview",
        ),
        required=True,
    )
    parser.add_argument("--provider", default="openai_codex")
    parser.add_argument("--model", default="codex_direct")
    parser.add_argument("--prompt-id", default="shortform_judge_v10_ko")
    parser.add_argument(
        "--execution-mode",
        choices=("independent_requests", "direct_single_session"),
        required=True,
    )
...
def main() -> None:
    ...
    for candidate in candidates:
        ...
        raw = build_raw(
            dimension_by_id[candidate_id],
            source_salience_policy=args.source_salience_policy,
        )
    ...
    write_jsonl(
        args.output_dir / f"{args.output_prefix}_judgments.jsonl",
        raw_rows,
    )
    write_csv(
        args.output_dir / f"{args.output_prefix}_scores.csv",
        score_rows,
    )

    scores = [float(row["judge_score_100"]) for row in score_rows]
    frequencies = Counter(scores)
    production_equivalent = args.execution_mode == "independent_requests"
    summary = {
        "judge_run_id": args.run_id,
        ...
        "production_equivalent_limitation": (
            ...
            if production_equivalent
                ...
                "Candidates were assessed one at a time in a label-blind direct "
                "Codex session, not in isolated model requests. Treat this as a "
                "blind direct pilot rather than a deployment-equivalent run."
            )
        ),
