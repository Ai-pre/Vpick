...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run the structurally blind v7 reference Judge with mR3 on GPU."
    )
    ...
    parser.add_argument(
        "--input-modality",
        default="yt_dlp_transcript_only_structurally_blind",
    )
    ...
    run_id = args.run_id or (
        f"mr3_qwen3_8b_{device_id}_{args.dtype}_reference_judge_v7"
    )
    ...
    for index, row in enumerate(candidates, start=1):
        ...
        if args.resume and raw_path.exists():
            ...
            judgment = raw_record.get("judgment")
        else:
            raw_record = {}
            judgment = None
        ...
        if judgment is not None:
            ...
            for attempt_index in range(max(0, args.format_retries) + 1):
                    ...
                    attempt_messages.append(
                        {
                            "role": "user",
                            "content": (
                                "출력 형식 오류를 수정하십시오. 분석 과정이나 "
                                "마크다운 없이, 앞서 요구한 스키마의 유효한 JSON "
                                "객체 하나만 출력하십시오."
                            ),
                        }
                    )
                ...
                model_inputs = tokenizer([chat_text], return_tensors="pt").to(
                    args.device
                )
                attempt_input_tokens = int(model_inputs.input_ids.shape[-1])
                ...
                attempt_elapsed_sec = time.perf_counter() - started
                output_ids = generated_ids[0, attempt_input_tokens:]
                attempt_generated_tokens = int(output_ids.shape[-1])
                raw_text = tokenizer.decode(output_ids, skip_special_tokens=False)
                attempts.append(
                    {
                        "attempt_index": attempt_index,
                        "input_tokens": attempt_input_tokens,
                        "generated_tokens": attempt_generated_tokens,
            ...
            if judgment is None:
                raise ValueError(
                    f"No valid judgment JSON after {len(attempts)} attempts "
